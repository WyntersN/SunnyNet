...
func (t *Transport) RoundTrip(req *Request) (*Response, error) {
	...
	success = js.FuncOf(func(this js.Value, args []js.Value) any {
		...
		case clHeader != "":
			...
			if err != nil {
				errCh <- fmt.Errorf("github.com/WyntersN/SunnyNet/src/http: ill-formed Content-Length header: %v", err)
				return nil
			}
			if cl < 0 {
				// Content-Length values less than 0 are invalid.
				// See: https://datatracker.ietf.org/doc/html/rfc2616/#section-14.13
				errCh <- fmt.Errorf("github.com/WyntersN/SunnyNet/src/http: invalid Content-Length header: %q", clHeader)
				return nil
			}
			contentLength = cl
		default:
	...
	failure = js.FuncOf(func(this js.Value, args []js.Value) any {
		success.Release()
		failure.Release()
		errCh <- fmt.Errorf("github.com/WyntersN/SunnyNet/src/http: fetch() failed: %s", args[0].Get("message").String())
		return nil
	})
...
var errClosed = errors.New("github.com/WyntersN/SunnyNet/src/http: reader is closed")
